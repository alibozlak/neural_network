...
/// `Layer::summary` prints the raw matrix dimensions, so the counts it reports still
/// contain the bias row and the bias column, and the two labels are swapped as well.
/// For a model of 2 features and the layers `[3, 1]` it prints
/// `Layer_1: Unit_Count: 4, weight_count: 3` instead of
/// `Layer_1: Unit_Count: 3, weight_count: 2`.
#[test]
#[ignore = "known bug: Layer::summary does not subtract the bias row/column"]
fn summary_reports_the_requested_unit_count_of_every_layer() {
    let summary = SequentialModel::new(2, &layer_requests(&[3, 1])).summary();
    ...
    assert_eq!(
        ...
        "Layer_Count: 2\n\
         Layer_1: Unit_Count: 3, weight_count: 2\n\
         Layer_2: Unit_Count: 1, weight_count: 3\n"
    );
}
